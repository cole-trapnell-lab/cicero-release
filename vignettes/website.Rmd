...
1. Download and install Monocle 3 and the Monocle 3 branch of Cicero from 
Github:
...
agg_cds <- detect_genes(agg_cds)
agg_cds <- estimate_size_factors(agg_cds)


mouse_rowdata <- readRDS("mouse_rowdata")
mouse_coldata <- readRDS("mouse_coldata")
mouse_exprs <- readRDS("mouse_exprs")

mouse_cds <- new_cell_data_set(mouse_exprs, cell_metadata = mouse_coldata, gene_metadata = mouse_rowdata)
...
```{r}
mouse_expr <- readRDS("~/Downloads/atac_matrix.binary.qc_filtered.rds")
mouse_rowdata <- read.table("~/Downloads/atac_matrix.binary.qc_filtered.peaks.txt")
mouse_coldata <- read.table("~/Downloads/atac_matrix.binary.qc_filtered.cells.txt")


cell_meta <- read.table("~/Downloads/cell_metadata.txt",sep="\t", header=T)
mouse_coldata <- cbind(mouse_coldata, cell_meta)
row.names(mouse_coldata) <- mouse_coldata$V1
row.names(mouse_rowdata) <- mouse_rowdata$V1

mouse_cds <- new_cell_data_set(mouse_expr, cell_metadata = mouse_coldata[colnames(mouse_expr),,drop=FALSE], gene_metadata = mouse_rowdata[row.names(mouse_expr),, drop=FALSE])

agg_cds <- aggregate_nearby_peaks(mouse_cds, distance = 10000)
agg_cds <- detect_genes(agg_cds)
agg_cds <- estimate_size_factors(agg_cds)
```
...
agg_cds <- preprocess_cds(agg_cds, residual_model_formula_str = "~num_genes_expressed")
agg_cds <- reduce_dimension(agg_cds, 
          reduction_method = 'UMAP')
plot_cells(agg_cds, color_cells_by = "timepoint")
```
